...
def walk_files(main_file, path, package, imports=None):
    ...
    if not desc_file and package == "WUProtos":
        print("Can't compile..")
        print("File: '%s'" % path)
        print("Please place the file in 'src/WUProtos/' in a sub-directory.")
        exit()
...
if desc_file:
    ...
    with open(root_package_file_path, 'a') as root_package_file:
        walk_files(root_package_file, proto_path, "WUProtos", created_packages)

    commands.append(
        """{0} --include_imports --proto_path="{1}" --descriptor_set_out="{2}" "{3}\"""".format(
            protoc_path,
            tmp_path,
            os.path.abspath(out_path + "/WUProtos.desc"),
            os.path.join(tmp_path, "WUProtos.proto")
        )
    )
else:
    if lang == "go":
